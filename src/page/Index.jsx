...
const Index = () => {
    ...
    useEffect(() => {
        setPokemons(PokeData.filter(x => x.name.fr.toLowerCase().includes(pokeSearch.toLowerCase()) || x.pokedexId == pokeSearch))
    }, [pokeSearch])

    return (
        <div className='d-flex flex-col flex-1 overflow-hidden'>
            <input style={{margin: "5px 35px"}} placeholder={"Chercher un pokemon"} type="text" value={pokeSearch} onChange={(inp) => setPokeSearch(inp.target.value)}/>
